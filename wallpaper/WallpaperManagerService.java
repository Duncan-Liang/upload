...
public class WallpaperManagerService extends IWallpaperManager.Stub {
    ...
    //add by youxiaoyan for SystemUI 20160309 -s
    private static final boolean DEBUG_DDS = true;
    private static final String WALLPAPER_DDS_PAD = "wallpaper-dds-pad";
    private static final String WALLPAPER_INFO_DDS_PAD = "wallpaper_info_dds_pad.xml";

    private static final int PHONE_MODE = 1;
    private static final int PAD_MODE = 2;
    private static final int PHONE_STATE = 3;
    private static final int PAD_STATE = 4;

    private static final int DELAY_CLEAR_WALLPAPER = 3000;

    private String mPhoneName = "";
    private ComponentName mPhoneComponent = null;
    private String mPadName = "";
    private ComponentName mPadComponent = null;

    private int mCurrentScreenSize = -1;
    private boolean mDimensionUpdate = false;
    ...
    private final Handler mHandlerAsus = new Handler() {
        public void handleMessage(Message msg) {
            synchronized (mLock) {
                handleDdsStateLocked(msg.what);
            }
        }
    };

    static final String LOCKSCREEN = "lockscreen";
    static final String LOCKSCREEN_PAD = "lockscreen_pad";
    static final String LOCKSCREEN_PHONE = "lockscreen_phone";

    static final String ACTION_LOCKSCREEN_WALLPAPER_CHANGED = "asus.intent.action.LOCKSCREEN_WALLPAPER_CHANGED";
    /**
    ...
    // Callback function for lockscreen wallpaper
    private IWallpaperManagerCallback mLockscreenWallpaperCallback = null;
    //add by youxiaoyan for SystemUI 20160309 -e
    ...
    private class WallpaperObserver extends FileObserver {
        ...
        //add by youxiaoyan for SystemUI 20160309 -s
        final File mLockscreenPhoneWallpaperFile;
        final File mLockscreenPadWallpaperFile;
        //add by youxiaoyan for SystemUI 20160309 -e
        ...
        public WallpaperObserver(WallpaperData wallpaper) {
            ...
            //add by youxiaoyan for SystemUI 20160309 -s
            mLockscreenPhoneWallpaperFile = new File(mWallpaperDir, LOCKSCREEN_PHONE);
            mLockscreenPadWallpaperFile = new File(mWallpaperDir, LOCKSCREEN_PAD);
            //add by youxiaoyan for SystemUI 20160309 -e
        }
        ...
        public void onEvent(int event, String path) {
            ...
            synchronized (mLock) {
                ...
                if( WIND_DEF_ASUS_SYSTEMUI ) {
                    // Clear previous locscreen wallpaper when file changed
                    if (mLockscreenPhoneWallpaperFile.equals(changedFile) || mLockscreenPadWallpaperFile.equals(changedFile)) {
                        if (mLockscreenWallpaperCallback != null) {
                            try {
                                mLockscreenWallpaperCallback.onLockscreenWallpaperChanged();
                            } catch (RemoteException e) {
                                // The RemoteCallbackList will take care of removing
                                // the dead object for us.
                            }
                        }
                        broadcastLockscreenWallpaperChanged();
                    }
                }
                //add by youxiaoyan for SystemUI 20160309 -e                
            }
    ...
    class WallpaperData {
        ...
        boolean isProtectedDdsPad = false;
        //add by sunxiaolong@wind-mobi.com for theme patch end
    ...
    public void systemRunning() {
        ...
        mContext.registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                //add by youxiaoyan for SystemUI 20160309 -s
                // +++
                else if ( WIND_DEF_ASUS_SYSTEMUI && Intent.ACTION_CONFIGURATION_CHANGED.equals(action)) {
                    if (!WallpaperUtilities.isSupportDds()) return;
                    int newScreenSize = WallpaperUtilities.getScreenSize(mContext);
                    if(mCurrentScreenSize != newScreenSize) {
                        mCurrentScreenSize = newScreenSize;
                        if (WallpaperUtilities.isScreenSizePad(newScreenSize)) {
                            setScreenSizeMode(PAD_MODE);
                        } else {
                            setScreenSizeMode(PHONE_MODE);
                        }

                        // do not revert to build in wallpaper when process killed by dds
                        if (mRunnable != null) {
                            if (DEBUG_DDS) Slog.v(TAG, "onReceive: clearWallpaperLocked dds cancel");
                            mHandlerAsus.removeCallbacks(mRunnable);
                            mRunnable = null;
                        }
                        // Reset lockscreen wallpaper
                        if (mLockscreenWallpaperCallback != null) {
                            try {
                                Slog.d(TAG, "reset lockscreen wallpaper");
                                mLockscreenWallpaperCallback.onLockscreenWallpaperChanged();
                            } catch (RemoteException e) {
                                // The RemoteCallbackList will take care of removing
                                // the dead object for us.
    ...
    void onRemoveUser(int userId) {
        ...
        synchronized (mLock) {
            ...
            //add by youxiaoyan for SystemUI 20160309 -s
            if( WIND_DEF_ASUS_SYSTEMUI ) {
                File lockscreenwallpaperPadFile = new File(getWallpaperDir(userId), LOCKSCREEN_PAD);
                lockscreenwallpaperPadFile.delete();
                File lockscreenwallpaperPhoneFile = new File(getWallpaperDir(userId), LOCKSCREEN_PHONE);
                lockscreenwallpaperPhoneFile.delete();
            }
            //add by youxiaoyan for SystemUI 20160309 -e
        }
    }
        ...
        synchronized (mLock) {
            ...
            try {
                if (outParams != null) {
                    ...
                    //add by sunxiaolong@wind-mobi.com for theme patch start
                    if(HAS_ASUS_THEME) {
                        outParams.putBoolean("protected", WallpaperUtilities.isDdsPad(mContext) ?
                                wallpaper.isProtectedDdsPad : wallpaper.isProtected);
                        outParams.putInt("uid", callingUid);
                    }
                    //add by sunxiaolong@wind-mobi.com for theme patch end
                }
    ...
    //add by youxiaoyan for SystemUI 20160309 -s
    /*
     * User may has set lockscreen before, so we need make it still work
     */
    public void upgradeLockscreenWallpaper() {
        ...
        String mode = getLockscreenPath(userId);
        File old_file = new File(getWallpaperDir(userId), LOCKSCREEN);
        if (old_file.exists()) {
            boolean reuslt = old_file.renameTo(new File(getWallpaperDir(userId), mode));
            Slog.d(TAG, "Update lockscreen wallpaper path result: " + reuslt);
        }
    }

    private String getLockscreenPath(int userId) {
        if ((mContext.getResources().getConfiguration().screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK)
                > Configuration.SCREENLAYOUT_SIZE_NORMAL) {
            return LOCKSCREEN_PAD;
        } else {
            return LOCKSCREEN_PHONE;
        }
    }
    ...
    public ParcelFileDescriptor getLockscreenWallpaper(IWallpaperManagerCallback cb) {
        ...
        String mode = getLockscreenPath(userId);
        Slog.d(TAG, "getLockscreenWallpaper, mode=" + mode);
        try {
            mLockscreenWallpaperCallback = cb;
            File f = new File(getWallpaperDir(userId), mode);
            if (!f.exists()) {
                return null;
            }
            return ParcelFileDescriptor.open(f, MODE_READ_ONLY);
            ...
            /* Shouldn't happen as we check to see if the file exists */
            Slog.w(TAG, "Error getting lockscreen wallpaper", e);
        }
        return null;
    }
    //add by youxiaoyan for SystemUI 20160309 -e
 ...
 public ParcelFileDescriptor setWallpaper(String name, String callingPackage) {
        ...
        synchronized (mLock) {
            ...
            //add by sunxiaolong@wind-mobi.com for theme patch start
            if(HAS_ASUS_THEME) {
                if (WallpaperUtilities.isDdsPad(mContext)) {
                    wallpaper.isProtectedDdsPad = false;
                } else {
                    wallpaper.isProtected = false;
                }
            }
            //add by sunxiaolong@wind-mobi.com for theme patch end			
            final long ident = Binder.clearCallingIdentity();
    ...
    //add by sunxiaolong@wind-mobi.com for theme patch begin
    public ParcelFileDescriptor setWallpaperProtected(String name, String callingPackage) {
        checkPermission(android.Manifest.permission.SET_WALLPAPER);
        if (!isWallpaperSupported(callingPackage)) {
            return null;
        }
        synchronized (mLock) {
            ...
            WallpaperData wallpaper = getWallpaperSafeLocked(userId);
            if (WallpaperUtilities.isDdsPad(mContext)) {
                wallpaper.isProtectedDdsPad = true;
            } else {
                wallpaper.isProtected = true;
            }
            final long ident = Binder.clearCallingIdentity();
            try {
                ParcelFileDescriptor pfd = updateWallpaperBitmapLocked(name, wallpaper);
    ...
    //add by sunxiaolong@wind-mobi.com for theme patch end	
    //add by youxiaoyan for SystemUI 20160309 -s
    public ParcelFileDescriptor setWallpaperforLockscreen() {
        //int userId = UserHandle.getCallingUserId();
        int userId = mCurrentUserId;
        String mode = getLockscreenPath(userId);
        if (mode == null) return null;

        File dir = getWallpaperDir(userId);
        if (!dir.exists()) {
            dir.mkdir();
        ...
        try {
            File file = new File(dir, mode);
            fd = ParcelFileDescriptor.open(file,
                    MODE_CREATE|MODE_READ_WRITE|MODE_TRUNCATE);
            //restore lockscreen wallpaper file to its default SELinux security context
            if (!SELinux.restorecon(file)) {
                return null;
            }
            return fd;
        } catch (FileNotFoundException e) {
            Slog.w(TAG, "Error setting wallpaper for lockscreen", e);
        }
        return null;
    }
    //add by youxiaoyan for SystemUI 20160309 -e
    ...
    //add by youxiaoyan for SystemUI 20160309 -s
    private void broadcastLockscreenWallpaperChanged() {
        Slog.d(TAG, "lockscreen wallpaper changed,broadcast intent");
        final Intent intent = new Intent(ACTION_LOCKSCREEN_WALLPAPER_CHANGED);
        ...
        /*if ( WIND_DEF_ASUS_SYSTEMUI ) {
            try {
                if ((mContext.getResources().getConfiguration().screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK) > Configuration.SCREENLAYOUT_SIZE_NORMAL) {
                    Settings.System.putIntForUser(mContext.getContentResolver(), Settings.System.SET_PAD_WALLPAPER_FOR, type, UserHandle.USER_CURRENT);
                } else {
                    Settings.System.putIntForUser(mContext.getContentResolver(), Settings.System.SET_PHONE_WALLPAPER_FOR, type, UserHandle.USER_CURRENT);
                }
            } finally {
                Binder.restoreCallingIdentity(ident);
            }
    ...
    private void saveSettingsLocked(WallpaperData wallpaper) {
        ...
        try {
            ...
            //add by sunxiaolong@wind-mobi.com for theme patch start
            if(HAS_ASUS_THEME) {
                out.attribute(null, "isProtected", Boolean.toString(wallpaper.isProtected));
                out.attribute(null, "isProtectedDdsPad", Boolean.toString(wallpaper.isProtectedDdsPad));
            }
            //add by sunxiaolong@wind-mobi.com for theme patch end
            if (wallpaper.wallpaperComponent != null
            ...
            do {
                ...
                if (type == XmlPullParser.START_TAG) {
                    ...
                    if ("wp".equals(tag)) {
                        ...
                        //add bu sunxiaolong@wind-mobi.com for theme patch start
                        if(HAS_ASUS_THEME) {
                            wallpaper.isProtected = Boolean.parseBoolean(parser.getAttributeValue(null, "isProtected"));
                            wallpaper.isProtectedDdsPad = Boolean.parseBoolean(parser.getAttributeValue(null, "isProtectedDdsPad"));
                        }
                        //add bu sunxiaolong@wind-mobi.com for theme patch end
    ...
    */
    ...
    //add by youxiaoyan for SystemUI 20160309 -s
    // +++
    private void initialScreenChangedVariable(int userId) {
        //initialize phone variable
        if (DEBUG_DDS) Slog.v(TAG, "initialScreenChangedVariable: userId = " + userId);
        //loadSettingsLocked(userId, false);
        loadSettingsLocked(userId);//modify by youxiaoyan in asus_patch 20160311

        WallpaperData wallpaper = mWallpaperMap.get(userId);
        if (wallpaper == null) return;

        mPhoneName = wallpaper.name;
        mPhoneComponent = wallpaper.nextWallpaperComponent == null
                ? mImageWallpaper : wallpaper.nextWallpaperComponent;
        mPhoneWidth = wallpaper.width;
        mPhoneHeight = wallpaper.height;
        if (DEBUG_DDS) Slog.v(TAG, "initialScreenChangedVariable: phoneName = " + mPhoneName + " , phoneComponent = " + mPhoneComponent);

        //initialize pad variable
        //loadSettingsLocked(userId, true);
        loadSettingsLocked(userId);//modify by youxiaoyan in asus_patch 20160311
        wallpaper = mWallpaperMap.get(userId);
        mPadName = wallpaper.name;
        mPadComponent = wallpaper.nextWallpaperComponent == null
                ? mImageWallpaper : wallpaper.nextWallpaperComponent;
        mPadWidth = wallpaper.width;
        mPadHeight = wallpaper.height;
        if (DEBUG_DDS) Slog.v(TAG, "initialScreenChangedVariable: padName = " + mPadName + " , padComponent = " + mPadComponent);

        mCurrentScreenSize = WallpaperUtilities.getScreenSize(mContext);
        if (DEBUG_DDS) Slog.v(TAG, "initialScreenChangedVariable: currentScreenSize : " + mCurrentScreenSize);
    }

    private void setScreenSizeMode(int mode) {
        if(mHandlerAsus.hasMessages(PHONE_STATE)) mHandlerAsus.removeMessages(PHONE_STATE);
        if(mHandlerAsus.hasMessages(PAD_STATE)) mHandlerAsus.removeMessages(PAD_STATE);
        switch (mode) {
            case PHONE_MODE :
                mHandlerAsus.sendEmptyMessage(PHONE_STATE);
                break;
            case PAD_MODE :
                mHandlerAsus.sendEmptyMessage(PAD_STATE);
                break;
            default :
                break;
        }
    ...
    /**
     * Force to bind livewallpaper(fix MyTree bug(background fit screen size)when DDS).
     */
    private void setWallpaperComponent(ComponentName name,boolean force) {
        if (DEBUG) Slog.v(TAG, "setWallpaperComponent name=" + name);
        int userId = UserHandle.getCallingUserId();
        WallpaperData wallpaper = mWallpaperMap.get(userId);
        if (wallpaper == null) {
        ...
        synchronized (mLock) {
            ...
            try {
                wallpaper.imageWallpaperPending = false;
                //bindWallpaperComponentLocked(name, force, true, wallpaper,null, false);
                bindWallpaperComponentLocked(name, force, true, wallpaper,null);//modify by youxiaoyan in asus_patch 20160311
            } finally {
                Binder.restoreCallingIdentity(ident);
            }
        }
    ...
    private void saveDdsComponentLocked(WallpaperData wallpaper) {
        if (WallpaperUtilities.isDdsPad(mContext)) {
            if (DEBUG_DDS) Slog.v(TAG, "saveDdsComponentLocked: padName = " + wallpaper.name + " , padComponent = " + wallpaper.wallpaperComponent);
            mPadName = wallpaper.name;
            mPadComponent = wallpaper.wallpaperComponent;
        } else {
            if (DEBUG_DDS) Slog.v(TAG, "saveDdsComponentLocked: phoneName = " + wallpaper.name + " , phoneComponent = " + wallpaper.wallpaperComponent);
            mPhoneName = wallpaper.name;
            mPhoneComponent = wallpaper.wallpaperComponent;
        }
    }

    private void handleDdsStateLocked(int state) {
        if (DEBUG_DDS) Slog.v(TAG, "handleDdsStateLocked: start state = " + state);
        int userId = UserHandle.getCallingUserId();
        WallpaperData wallpaper = mWallpaperMap.get(userId);
        switch (state) {
        case PHONE_STATE :
            try {
                mContext.getPackageManager().getPackageInfo(mPhoneComponent.getPackageName(), 0);
            } catch (NameNotFoundException e) {
                if (DEBUG_DDS) Slog.v(TAG, "handleDdsStateLocked: component not found = " + mPhoneComponent);
                mPhoneComponent = mImageWallpaper;
            }

            if (mImageWallpaper.equals(mPhoneComponent)) {
                //Speed up ImageWallpaper service restart.
                Intent it = new Intent();
                it.setComponent(mImageWallpaper);
                mContext.startService(it);

                wallpaper.name = mPhoneName;
                //When pad component and component mode are both static wallpaper, don't re-bind ImageWallpaper service.
                //When pad component is live wallpaper, bind ImageWallpaper service.
                if (!mImageWallpaper.equals(mPadComponent)) {
                    if (DEBUG_DDS) Slog.v(TAG, "handleDdsStateLocked: Pad component is not ImageWallpaper, bind to ImageWallpaper");
                    //bindWallpaperComponentLocked(mImageWallpaper, true, false, wallpaper, null, false);
                    bindWallpaperComponentLocked(mImageWallpaper, true, false, wallpaper, null);//modify by youxiaoyan in asus_patch 20160311
                } else {
                    try {
                        mDimensionUpdate = true;
                        setDimensionHints(mPhoneWidth, mPhoneHeight, mContext.getOpPackageName());
                    } catch (RemoteException e) {
                        Slog.w(TAG,"handleDdsStateLocked: setDimension fail");
                    } finally {
                        mDimensionUpdate = false;
                    }
                }
            } else {
                if (DEBUG_DDS) Slog.v(TAG, "handleDdsStateLocked: Bind to LiveWallpaper : " + mPhoneComponent);
                wallpaper.name = mPhoneName;
                setWallpaperComponent(mPhoneComponent, true);
            }
            break;
        case PAD_STATE :
            try {
                mContext.getPackageManager().getPackageInfo(mPadComponent.getPackageName(), 0);
            } catch (NameNotFoundException e) {
                if (DEBUG_DDS) Slog.v(TAG, "handleDdsStateLocked: component not found = " + mPadComponent);
                mPadComponent = mImageWallpaper;
            }
            ...
            if (mImageWallpaper.equals(mPadComponent)) {
                ...
                wallpaper.name = mPadName;
                //When pad component and phone component are both static wallpaper, don't re-bind ImageWallpaper service.
                //When phone component is live wallpaper, bind ImageWallpaper service.
                if (!mImageWallpaper.equals(mPhoneComponent)) {
                    if (DEBUG_DDS) Slog.v(TAG, "handleDdsStateLocked: Phone component is not ImageWallpaper, bind to ImageWallpaper");
                    //bindWallpaperComponentLocked(mImageWallpaper, true, false, wallpaper ,null, false);
                    bindWallpaperComponentLocked(mImageWallpaper, true, false, wallpaper ,null);//modify by youxiaoyan in asus_patch 20160311
                } else {
                    try {
                        mDimensionUpdate = true;
                        setDimensionHints(mPadWidth, mPadHeight, mContext.getOpPackageName());
                    } catch (RemoteException e) {
                        Slog.w(TAG, "handleDdsStateLocked: setDimension fail");
                    } finally {
                        mDimensionUpdate = false;
                ...
                if (DEBUG_DDS) Slog.v(TAG, "handleDdsStateLocked: Bind to LiveWallpaper : " + mPadComponent);
                wallpaper.name = mPadName;
                setWallpaperComponent(mPadComponent, true);
            }
            break;
        default:
            break;
        }
        if (DEBUG_DDS) Slog.v(TAG, "handleDdsStateLocked: end state = " + state);
    }
    // --- 

    public void updateWallpaperSetting(int type) {
        final long ident = Binder.clearCallingIdentity();
        try {
            if ((mContext.getResources().getConfiguration().screenLayout & Configuration.SCREENLAYOUT_SIZE_MASK)
                    > Configuration.SCREENLAYOUT_SIZE_NORMAL) {
                Settings.System.putInt(mContext.getContentResolver(), Settings.System.SET_PAD_WALLPAPER_FOR, type);
            } else {
                Settings.System.putInt(mContext.getContentResolver(), Settings.System.SET_PHONE_WALLPAPER_FOR, type);
            }
        } finally {
            Binder.restoreCallingIdentity(ident);
        }
